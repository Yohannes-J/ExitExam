  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        {/* Result card */}
        <div className={`rounded-2xl shadow-xl p-8 mb-6 text-white text-center ${
          ...
          <h1 className="text-3xl font-bold mb-1">{result.passed ? 'Congratulations!' : 'Better Luck Next Time'}</h1>
          ...
          <div className="grid grid-cols-3 gap-4 max-w-sm mx-auto">
            ...
            <div className="bg-white/20 rounded-xl p-3">
              <div className="text-3xl font-bold">{result.score}/{result.totalPoints}</div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          ...
          <div className="space-y-4">
            {exam.questions?.map((q, i) => {
              ...
              return (
                <div key={i} className={`rounded-xl border-2 p-4 ${isCorrect ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                  ...
                  <div className="space-y-1.5 ml-8">
                    {q.options.map((opt, j) => (
                      <div key={j} className={`text-sm px-3 py-1.5 rounded-lg ${
                        ...
                        {opt}
                        {j === q.correctIndex && <span className="ml-2 text-green-700">← Correct</span>}
                        {j === selected && !isCorrect && <span className="ml-2 text-red-700">← Your answer</span>}
                      </div>
                    ))}
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate('/dashboard')} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-xl transition">
            Back to Dashboard
          </button>
          <button onClick={() => navigate('/results')} className="flex-1 border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold py-3 rounded-xl transition">
            All Results
          </button>
        </div>
      </div>
